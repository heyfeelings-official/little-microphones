...
async function createRecordingElement(recordingData, questionId, allIds) {
    ...
    playerContainer.style.cssText = `width: 100%; height: 48px; position: relative; background: ${backgroundColor}; border-radius: 122px; display: flex; align-items: center; padding: 0 8px; box-sizing: border-box;`;
...
async function deleteRecording(recordingId, questionId, elementToRemove) {
    ...
    try {
        ...
        const recordingData = {
            id: recordingId,
                            cloudUrl: `https://little-microphones.b-cdn.net/${lmid}/${world}/${recordingId}.webm`
        };
    ...
    return new Promise((resolve, reject) => {
        ...
        eventSource.onmessage = (event) => {
            try {
                ...
                console.log(`📊 SSE event for job ${jobId}: status ${data.status || 'connecting'}`);
...
async function loadRecordingsFromCloud(questionId, world, lmid) {
    try {
        ...
        const recordings = cloudRecordings.map(cloudRec => ({
                            id: cloudRec.filename.replace('.webm', '').replace('.mp3', ''),
            questionId: questionId,
