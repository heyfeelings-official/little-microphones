 ...
 */
export async function createRecordingElement(recordingData, questionId, getAudioSource, deleteRecording, dispatchUploadStatusEvent) {
 ...
 */
export function createRecordingPlaceholder(questionId) {
 ...
 */
export function updateUploadStatusUI(uploadElement, deleteElement, recordingData, currentInterval) {
 ...
 */
export function showRadioProgramModal(message, progress = 0) {
    hideRadioProgramModal(); // Remove any existing modal
    ...
    title.style.cssText = 'margin: 0 0 10px 0; color: #333; font-size: 24px;';
    title.textContent = 'Generating Radio Program 🎵';
    
    const statusMessage = document.createElement('p');
    statusMessage.id = 'radio-status-message';
    statusMessage.style.cssText = 'margin: 0 0 20px 0; color: #666; font-size: 16px;';
    ...
    progressFill.id = 'radio-progress-fill';
    ...
    progressText.id = 'radio-progress-text';
    progressText.style.cssText = 'font-size: 14px; color: #999; margin-top: 10px;';
 ...
 */
export function updateRadioProgramProgress(message, progress, details = '') {
    const statusMessage = document.getElementById('radio-status-message');
    const progressFill = document.getElementById('radio-progress-fill');
    const progressText = document.getElementById('radio-progress-text');
    ...
    if (progressText) {
        progressText.textContent = `${progress}% complete${details ? ` - ${details}` : ''}`;
    }
    
    // Add some fun rotating messages
    const spinner = document.getElementById('radio-spinner');
    if (spinner && progress > 0) {
        const updateSpinner = () => {
            const messages = ['🎵', '🎶', '🎤', '📻', '🎧'];
            const randomMessage = messages[Math.floor(Math.random() * messages.length)];
            spinner.innerHTML = `<div style="line-height: 52px; font-size: 30px;">${randomMessage}</div>`;
 ...
 */
export function hideRadioProgramModal() {
    const modal = document.getElementById('radio-program-modal');
    if (modal) {
        modal.remove();
    }
    
    const successModal = document.getElementById('radio-success-modal');
    if (successModal) {
        successModal.remove();
    }
}
 ...
 * @param {number} totalRecordings - Total recordings count
 */
export function showRadioProgramSuccess(audioUrl, world, lmid, questionCount, totalRecordings) {
    ...
    title.style.cssText = 'margin: 0 0 20px 0; color: #333; font-size: 28px;';
    title.textContent = '🎉 Your Radio Program is Ready!';
    
    const subtitle = document.createElement('p');
    subtitle.style.cssText = 'margin: 0 0 30px 0; color: #666; font-size: 16px;';
    subtitle.textContent = `Generated from ${totalRecordings} recordings across ${questionCount} questions in ${world}`;
    
    const audioPlayer = document.createElement('audio');
    audioPlayer.src = audioUrl;
    audioPlayer.controls = true;
    audioPlayer.style.cssText = 'width: 100%; margin-bottom: 30px; border-radius: 8px;';
    ...
    `;
    downloadButton.onmouseover = () => downloadButton.style.background = '#0056b3';
    downloadButton.onmouseout = () => downloadButton.style.background = '#007AF7';
    downloadButton.onclick = () => {
        const link = document.createElement('a');
        link.href = audioUrl;
        link.download = `radio-program-${world}-${lmid}.mp3`;
        link.click();
    };
    ...
    closeButton.onclick = () => hideRadioProgramModal();
...
/**
 * Start fun animated status messages
 * @returns {number} Interval ID
 */
export function startFunStatusMessages() {
    const statusMessages = [
        "🎵 Mixing your voices...",
        "🎶 Adding musical magic...",
        "🎤 Polishing your performances...",
        "📻 Tuning the radio waves...",
        "🎧 Fine-tuning the audio...",
        "✨ Sprinkling some audio dust...",
        "🎼 Composing your masterpiece...",
        "🔊 Amplifying the awesomeness..."
    ];
    
    let messageIndex = 0;
    
    funStatusInterval = setInterval(() => {
        const statusElement = document.getElementById('radio-status-message');
        if (statusElement) {
    ...
    return funStatusInterval;
}

/**
 * Stop fun animated status messages
 */
export function stopFunStatusMessages() {
