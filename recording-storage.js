 ...
 */
export function withStore(type, callback) {
    return new Promise((resolve, reject) => {
        if (!db) {
 ...
 */
export function getRecordingFromDB(recordingId) {
 ...
 * @returns {Promise<void>}
 */
export function updateRecordingInDB(recordingData) {
    return withStore('recordings', (store, resolve, reject) => {
 ...
 */
export function loadRecordingsFromDB(questionId, world, lmid) {
    return withStore('recordings', (store, resolve, reject) => {
 ...
 * @returns {Promise<void>}
 */
export function saveRecordingToDB(recordingData) {
    return withStore('recordings', (store, resolve, reject) => {
 ...
 * @returns {Promise<void>}
 */
export function deleteRecordingFromDB(recordingId) {
    return withStore('recordings', (store, resolve, reject) => {
 ...
 * @returns {Promise<Array>} Array of recordings
 */
export function getAllRecordingsForWorldLmid(world, lmid) {
    return withStore('recordings', (store, resolve, reject) => {
 ...
 */
export function discoverQuestionIdsFromDB(world, lmid) {
    return new Promise(async (resolve, reject) => {
        try {
 ...
 */
export async function loadRecordingsFromCloud(questionId, world, lmid) {
    try {
 ...
 */
export async function uploadToBunny(recordingData, world, lmid) {
    try {
 ...
 */
export async function deleteFromBunny(recordingData, world, lmid, questionId) {
    try {
 ...
 */
export async function cleanupOrphanedRecordings(questionId, world, lmid) {
    try {
 ...
 */
export async function cleanupAllOrphanedRecordings(world, lmid) {
    try {
 ...
 */
export async function syncRecordingsWithCloud(world, lmid) {
    try {
        ...
        return {
            ...
            syncedCount,
            errorCount,
            message: `Synced ${syncedCount} recordings with ${errorCount} errors`
        };
    } catch (error) {
 ...
 * @param {string} world - World name (optional)
 * @param {string} lmid - LMID (optional)
 * @returns {Promise<Object>} Storage statistics
 */
export async function getStorageStats(world = null, lmid = null) {
    try {
        let recordings;
        ...
        const stats = {
            ...
            uploadedCount: recordings.filter(r => r.uploadStatus === 'uploaded').length,
            pendingCount: recordings.filter(r => r.uploadStatus === 'pending').length,
            failedCount: recordings.filter(r => r.uploadStatus === 'failed').length,
            withLocalBlob: recordings.filter(r => r.audio).length,
            withCloudUrl: recordings.filter(r => r.cloudUrl).length,
            uniqueQuestions: [...new Set(recordings.map(r => r.questionId))].length,
            uniqueWorlds: [...new Set(recordings.map(r => r.world))].length,
            uniqueLmids: [...new Set(recordings.map(r => r.lmid))].length
        };
